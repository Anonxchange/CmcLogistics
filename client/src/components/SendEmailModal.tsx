...
const getHtmlTemplate = (title: string, content: string, shipment: any) => {
...
Best regards,
CMC Logistics Team`,
    htmlContent: (shipment: any) => getHtmlTemplate(
      'Shipment Created Successfully!',
      `
        <p>Dear <strong>${shipment?.recipient_name || 'Valued Customer'}</strong>,</p>
...
export default function SendEmailModal({ isOpen, onClose, shipment }: SendEmailModalProps) {
  ...
  const [recipient, setRecipient] = useState(shipment?.recipient_email || '');
  const [subject, setSubject] = useState(emailTemplates.shipment_created.subject);
  const [body, setBody] = useState(emailTemplates.shipment_created.body);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const finalBody = useHtmlTemplate 
        ? emailTemplates[templateType as keyof typeof emailTemplates].htmlContent?.(shipment) || replacePlaceholders(body)
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Send shipment updates to customer for tracking number: {shipment?.tracking_number}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={useHtmlTemplate ? 'html' : 'text'} onValueChange={(v) => setUseHtmlTemplate(v === 'html')}>
                <SelectTrigger id="format">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="html">Professional HTML</SelectItem>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Available placeholders: [CUSTOMER_NAME], [TRACKING_NUMBER], [SERVICE_TYPE], [DELIVERY_DATE], [LOCATION], [TRACKING_URL]
            </p>
          </div>
          ...
          <div className="bg-muted p-4 rounded-lg space-y-2">
            ...
            <div className="text-sm space-y-1">
              <p><strong>To:</strong> {recipient || 'customer@example.com'}</p>
              <p><strong>Subject:</strong> {replacePlaceholders(subject)}</p>
              <div className="mt-2 p-3 bg-background rounded border">
                <pre className="whitespace-pre-wrap text-xs">{replacePlaceholders(body)}</pre>
              </div>
            </div>
          </div>
        </div>
